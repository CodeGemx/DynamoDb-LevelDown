...
/* @internal */
const serialize = (value) => getTransformerOrThrow(value).toDb(value);
exports.serialize = serialize;
/* @internal */
const deserialize = (value) => getTransformerOrThrow(value).fromDb(value);
...
const transformMapFrom = (value) => {
    ...
    for (const typedItem of value) {
        const item = exports.deserialize(typedItem);
        result.push(item);
    }
    return result;
};
const transformMapTo = (value) => {
    return value.map((item) => exports.serialize(item));
};
...
const TRANSFORMERS = [
    ...
    },
    {
        ...
        for: (value) => (utils_1.isBuffer(value) && value.length === 0) ||
    ...
    {
        for: (value) => utils_1.isBuffer(value) || ctorName(value) === 'Buffer' || dbotName(value) === 'B',
        toDb: (value) => ({ B: value }),
    ...
    },
    {
        ...
        toDb: (value) => ({ M: transformReduce(value, exports.serialize) }),
        fromDb: (value) => transformReduce(value.M, exports.deserialize),
    },
];
